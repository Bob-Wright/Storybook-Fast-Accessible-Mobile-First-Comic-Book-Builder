...
<head>
	...
	<title>Alt Images Alt Text Upload</title>
...
<h1 style="color:blue; text-align:center;">Alt Image Alt Text Files Upload</h1>
<!-- quick display of info about the upload requirements -->
<div class="msgBox"><h2 style="color:purple;">This page will let you select and upload alternate or "alt" text for each of the alternate page images included in the Comic.</h2>
<h3 style="color: #ff0040;">In order for Storybook Comic Builder to include alt text with a Comic image, a text file must be uploaded for each image. The alt text filename must match the associated image filename.</h3><h3 style="color: purple;">Storybook Comic Builder allows the upload of up to <b>
EOT2;
echo $head2;
echo '<span id="panelcount">' . $panelcount . '</span>';
$head3 = <<< EOT3
</b> alternate image alt text files for inclusion in the Comic.</h3></div><br>
...
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and upload the Alt Image Alt text files:<br><input type="file" accept=".txt,.TXT" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple="multiple" tabindex="1">
...
echo $head3;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltImg.php" title="return to uploading Alternate images" tabindex="4">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getAltImgCaptions.php" title="get captions for the alternate images" tabindex="3">Next ❯</a></p></nav>'.
		'</footer>';
$scr = <<< SCR1
      <script>
window.onload = function() {
           ...
           $("input[type = 'submit']").click(function(){
               ...
               if (parseInt(fileUpload.get(0).files.length) > panelcount){
                  alert("You are only allowed to upload " + panelcount + " Comic Alternate image Alt text files!");
				  window.location.replace("https://syntheticreality.net/Storybook/getAltImgText.php");
               }
            });
         };
      </script>
