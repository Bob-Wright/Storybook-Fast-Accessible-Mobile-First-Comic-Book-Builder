...
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and upload the Alt Image Alt text files:<br><input type="file" accept=".txt,.TXT" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple="multiple">
...
echo $head2;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltImg.php" title="return to uploading Alternate images">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i:s.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getComicCaptions.php" title="get captions for the comic images">Next ❯</a></p></nav>'.
		'</footer>';
?>
      <script>
         $(function(){
            $("input[type = 'submit']").click(function(){
               ...
               if (parseInt($fileUpload.get(0).files.length) > 24){
                  alert("You are only allowed to upload 24 alt image alt text files!");
