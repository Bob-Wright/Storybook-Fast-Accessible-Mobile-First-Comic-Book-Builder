...
$_SESSION['pageimage'] = 'altCaptions';
$head1 = <<< EOT1
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<title>Alt Img Captions Upload</title>
...
<h1 style="color:blue; text-align:center;">Alternate Image Caption Files Upload</h1>
<!-- quick display of info about the upload requirements -->
<div class="msgBox"><h2 style="color:purple;">This page will let you select and upload text captions for each of the alternate images included in the Comic.</h2>
<h3 style="color: #ff0040;">In order for Storybook Comic Builder to include a caption with an alternate Comic image, a text file must be uploaded for each captioned image. The text caption filename must match the associated image filename.</h3>
<h3 style="color:purple;">Storybook Comic Builder allows the upload of up to <b>
EOT2;
echo $head2;
echo '<span id="panelcount">' . $panelcount . '</span>';
$head3 = <<< EOT3
</b> alternate image caption text files for inclusion in the Comic.</h3></div><br><div class="infoBox"><p>Including captions is optional, and you may choose to caption some images and not others. However, including this caption text helps to provide information about the images to the audience, and greatly improves accessibility. Text content is also important for <em>SEO</em> or Search Engine Optimization. The best captions for search engines and viewers alike provide meaningful content about the associated image.</p>
...
echo $head3;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltImgText.php" title="return to uploading Alt text for alternate Images" tabindex="4">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getAltImgMP3.php" title="go to the audio file chooser" tabindex="3">Next ❯</a></p></nav>'.
		'</footer>';
$scr = <<< SCR1
      <script>
window.onload = function() {
           ...
           $("input[type = 'submit']").click(function(){
               ...
               if (parseInt(fileUpload.get(0).files.length) > panelcount){
                  alert("You are only allowed to upload " + panelcount + " Comic Alternate image Caption files!");
				  window.location.replace("https://syntheticreality.net/Storybook/getAltImgCaptions.php");
               }
            });
         };
      </script>
