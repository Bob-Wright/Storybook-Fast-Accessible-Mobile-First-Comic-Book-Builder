...
<head>
	...
	<title>Comic Alt Image MP3 Upload</title>
...
$(document).ready(function()
{
	...
	({
		...
		autoSubmit: true,
		vpb_server_url: "mp3Uploader.php" 
	});
});
</script>
<body>
<main class="pageWrapper" id="container">
<h1 style="color:blue; text-align:center;">Comic Audio Files Upload</h1>
<!-- quick display of info about the upload requirements -->

<div class="msgBox"><h2 style="color:purple;">This page will let you select and upload an <em>Audio File</em> for each of the Alternate Images included in the Comic.</h2>
<h3 style="color: #ff0040;">Storybook Comic Builder will optionally include an alternate image and an audio file for each Comic image. A typical use is to display an animated GIF file with audio on an image click. Alternate image audio filenames must match the associated image filename.</h3><h3 style="color: purple;">Storybook Comic Builder allows the upload of up to <b>
EOT2;
echo $head2;
echo '<span id="panelcount">' . $panelcount . '</span>';
$head3 = <<< EOT3
</b> alternate image audio files for inclusion in the Comic.</h3></div><br>
<div class="infoBox"><p>Storybook Comic Builder only allows the <b>MP3</b> audio filetype (case insensitive). Uploaded audio must be in this format and have a valid filename of up to 255 characters. At this time filenames must contain <b>ONLY</b> the English alphabetic characters <b>A</b> through <b>Z</b>, <b>a</b> through <b>z</b>, and the digits <b>0</b> through <b>9</b>.  Nonalphanumeric or special characters are limited to <em>spaces, underscores, dashes,</em> or <em>periods</em> and a few others as normal filename use allows.</p>
<p>Storybook Comic Builder will not upload content located on another site by a URL; you may only upload audio files from your device.</b> Audio files are limited to 10 Megabytes (10 Megabytes = 10,485,760 Bytes) maximum filesize. Note that all metadata content will be removed from uploaded audio files.</p></div>
<br>

<center>
<div class="infoBox">
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and upload the Comic Audio files:<br><input type="file" accept=".mp3,.MP3" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple="multiple" tabindex="1">
...
echo $head3;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltImgCaptions.php" title="return to uploading Alt image captions" tabindex="4">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./comicFonts.php" title="get fonts for caption text" tabindex="3">Next ❯</a></p></nav>'.
		'</footer>';
$scr = <<< SCR1
      <script>
window.onload = function() {
           ...
           $("input[type = 'submit']").click(function(){
               ...
               if (parseInt(fileUpload.get(0).files.length) > panelcount){
                  alert("You are only allowed to upload " + panelcount + " Comic audio files!");
				  window.location.replace("https://syntheticreality.net/Storybook/getAltImgMP3.php");
               }
            });
         };
      </script>
