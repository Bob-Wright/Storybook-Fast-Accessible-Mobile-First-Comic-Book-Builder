...
<div class="msgBox"><h2 style="color:purple;">This page will let you select and upload one <em>Page Background</em> image for the Comic page.</h2></div><br>
...
<p><strong>Storybook Comic Builder will not upload content from another site through a URL; you may only upload image files from your device.</strong> Image files are limited to 5 Megabytes (5 Megabytes = 5,242,880 Bytes) maximum filesize for <strong>JPG</strong> and <strong>PNG</strong> files. Best results will be obtained with the recommended image pixel dimensions. Note that all metadata content (<em>eg</em> EXIF data) will be removed from uploaded image files.</p></div>
<br>
<div class="msgBox"><p style="color:purple;">The specific image that is needed for the <em>Page Background</em> that will be displayed in the Comic should be a JPG or PNG format and be about 1200 to 1400 pixels on a side. The selected image will be scaled to fill the browser window when the page is being viewed, and it has reduced opacity for a softer background.</p></div>
<br>
<center>
<div class="msgBox">
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and upload a comic page background image:<br><input type="file" accept=".jpg,.JPG,.gif,.GIF,.png,.PNG" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple>
...
echo $head2;
	echo
	'<footer class="d-flex col-sm-12 flex-column align-items-center justify-content-end shadow-md #b0bec5 blue-grey lighten-3 px-sm-0 infoBox" id="ComicFooter">'.
	...
	-51z"/>
		...
		echo date ("F d Y H:i:s.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./index.php#bkgndimage" title="return to configuration settings">Next ❯</a></p></nav>'.
		'</footer>';
$scr = <<< SCR1
      <script>
window.onload = function() {
           $("input[type = 'submit']").click(function(){
               ...
               if (parseInt(fileUpload.get(0).files.length) > 1){
                  alert("You are only allowed to upload one Comic background image file!");
				  window.location.replace("https://syntheticreality.net/Storybook/getBkgnd.php");
               }
            });
         };
      </script>
