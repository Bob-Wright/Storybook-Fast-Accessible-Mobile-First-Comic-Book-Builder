...
//	alert("reached timeoutDelay");
	//window.location.replace("https://syntheticreality.net/Comics/Comics.html");
//};
...
if($_SESSION['Comicsaved'] == 0) {
	...
	'<label>Create the Comic:<br><input type="text" id="Comicname" name="Comicname" size="64" value="'.$Comicname.'" hidden>'.
	//'<input type="submit" hidden>'.
	'<input type="submit" value=" Click to create the Comic! "></label>'.
	'</form>'.
	'</section><br>';
	...
	'<section class="d-flex col-sm-11 flex-column align-items-center shadow-md #8bb19b lite-stylish-color-lite-G lighten-4 px-sm-0 msgBox"><h2 style="color: #6600d0;background: #ffff00;padding: 1vw;">The&nbsp;<span id="Comicname">'.$Comicname.'</span>&nbsp;Web Comic Files were created using the information that you supplied along with the ';
		...
		'<label>Share the comic on Facebook:<br><input class="checkbox" type="checkbox" id="shareCheckbox" name="shareCheckbox" value="1" checked hidden>'.
		'<input type="submit" value="&emsp; Share your Comic on Facebook.&emsp;"></label></form>';
...
/*	if($saveDB == false) {
		// clean the Storybook comic images database
		//$imageList = $ComicImages->listComic($Comicname, true);
		$imageList = $ComicImages->deleteComic($Comicname);
	}
*/	
}
...
if(($skippyLives == false) && (file_exists($Comics.$Comicname.'.html'))) {
	...
	if(file_exists($Comics.$Comicname.'OGIMG.png')) {
		$ziplist .= ', '. $Comics.$Comicname.'OGIMG.png';
	}
	...
	if(file_exists($Comics.'coversImg/'.$Comicname.'CARD.png')) {
		$ziplist .= ', '. $Comics.'coversImg/'.$Comicname.'CARD.png';
	}
	...
	if($saveC === false) {
		...
		if(file_exists($Comics.'coversImg/'.$Comicname.'CARD.png')) {
			$result = unlink($Comics.'coversImg/'.$Comicname.'CARD.png');}
		$folder = $Comics.$Comicname;
		rrmdir($folder);
	}

/*	if($saveDB === false) {
		// clean the Storybook database
		//$imageList = $ComicImages->listComic($Comicname, true);
		$imageList = $ComicImages->deleteComic($Comicname);
	}
*/
	if(file_exists($Comics.$Comicname.'.zip')) {
		...
		if($host == true) {
			...
			'<label>Download the '.$Comicname.' Archive.<br><input type="text" name="Comicname" id="Comicname" size="64" value="'.$Comicname.'" hidden>'.
			'<input style="width: 85vw;" type="submit" value="&emsp;Download the '.$Comicname.' Archive.&emsp;" id="Apply"></label>'.
			'</form></section>';
