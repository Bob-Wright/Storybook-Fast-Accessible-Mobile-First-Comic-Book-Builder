...
<p style="color: #ff0040;"><b>In order for Storybook Comic Builder to include a caption with a Comic image, a text file must be uploaded for each captioned image. The text caption filename must match the associated image filename.</b></p></div><br>
<div class="infoBox"><p>Storybook Comic Builder allows the inclusion of <em>text captions</em> for the images displayed in a Comic. Including captions is optional, and you may choose to caption some images and not others. However, including this caption text helps to provide information about the images to the audience, and greatly improves accessibility. Text content is also important for <em>SEO</em> or Search Engine Optimization. The best captions for search engines and viewers alike provide meaningful content about the associated image.</p>
...
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and Upload Image Caption files:<br><input type="file" accept=".txt,.TXT" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple="multiple">
...
echo $head2;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltImgText.php" title="return to uploading Alt text for Alt Images">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i:s.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./comicFonts.php" title="get the caption fonts for the comic">Next ❯</a></p></nav>'.
		'</footer>';
?>
      <script>
         $(function(){
            $("input[type = 'submit']").click(function(){
               ...
               if (parseInt($fileUpload.get(0).files.length) > 48){
                  alert("You are only allowed to upload 24 Comic image caption files!");
