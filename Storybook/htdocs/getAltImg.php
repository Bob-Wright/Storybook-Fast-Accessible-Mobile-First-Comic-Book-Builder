...
<p style="color: #ff0040;"><b>Storybook Comic Builder will optionally include an alternate image for each Comic image. A typical use is to display an animated GIF file on an image click. Alternate image filenames must match the associated image filename.<br>Storybook Comic Builder allows the upload of up to <b>24</b> alternate image files for inclusion in the Comic. Each image represents one full display screen in the Comic. The images will be ordered by their names.</b></p></div><br>
...
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and upload the Comic Alt Image files:<br><input type="file" accept=".jpg,.JPG,.gif,.GIF,.png,.PNG" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple="multiple">
...
echo $head2;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltText.php" title="return to uploading Alt text for the page images">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i:s.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getAltImgText.php" title="get Alt text for the alt images">Next ❯</a></p></nav>'.
		'</footer>';
?>
      <script>
         $(function(){
            $("input[type = 'submit']").click(function(){
               ...
               if (parseInt($fileUpload.get(0).files.length) > 24){
                  alert("You are only allowed to upload 24 Comic alternate image files!");
				  window.location.replace("https://syntheticreality.net/ComicBuilder/getAltImg.php");
               } 
            });
         });
      </script>
