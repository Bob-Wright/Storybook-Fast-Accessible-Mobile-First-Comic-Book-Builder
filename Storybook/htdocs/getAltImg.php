...
<head>
	...
	<title>Comic Alt Images Upload</title>
...
<h1 style="color:blue; text-align:center;">Comic Alternate Image Files Upload</h1>
<!-- quick display of info about the upload requirements -->

<div class="msgBox"><h2 style="color:purple;">This page will let you select and upload an <em>Alternate Image</em> for each of the images included in the Comic.</h2>
<h3 style="color: #ff0040;">Storybook Comic Builder will optionally include an alternate image for each Comic image. A typical use is to display an animated GIF file on an image click. Alternate image filenames must match the associated image filename.</h3><h3 style="color: purple;">Storybook Comic Builder allows the upload of up to <b>
EOT2;
echo $head2;
echo '<span id="panelcount">' . $panelcount . '</span>';
$head3 = <<< EOT3
</b> alternate image files for inclusion in the Comic. Each image represents one full display screen in the Comic. The images will be ordered by their names.</h3></div><br>
...
echo $head3;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getAltText.php" title="return to uploading Alt text for the page images" tabindex="4">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getAltImgText.php" title="get Alt text for the alt images" tabindex="3">Next ❯</a></p></nav>'.
		'</footer>';
$scr = <<< SCR1
      <script>
window.onload = function() {
           ...
           $("input[type = 'submit']").click(function(){
               ...
               if (parseInt(fileUpload.get(0).files.length) > panelcount){
                  alert("You are only allowed to upload " + panelcount + " Comic Alternate image files!");
				  window.location.replace("https://syntheticreality.net/Storybook/getAltimg.php");
               }
            });
         };
      </script>
