...
<h3 style="color: #ff0040;">In order for Storybook Comic Builder to include alt text with a Comic image, a text file must be uploaded for each image. The alt text filename must match the associated image filename.</h3>
<h3 style="color:purple;">Storybook Comic Builder allows the upload of up to <b>
EOT2;
echo $head2;
echo '<span id="panelcount">' . $panelcount . '</span>';
$head3 = <<< EOT3
</b> alt text files for inclusion in the Comic.</h3></div><br>
...
<form name="form_id" id="form_id" action="javascript:void(0);" enctype="multipart/form-data">
	<label>Select and upload the Comic Image Alt text files:<br><input type="file" accept=".txt,.TXT" name="vasplus_multiple_files" id="vasplus_multiple_files" multiple="multiple" tabindex="1">
	<input type="submit" value="Upload Selected Files" id="Upload" tabindex="1"></label>
</form>
<br></div>
<hr class="new4">
...
echo $head3;
	...
	'<nav id="ComicFooter"><p><a id="prevpagebutton" href="./getComic.php" title="return to uploading comic page images" tabindex="4">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	...
	-51z"/>
		...
		echo date ("F d Y H:i.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getComicCaptions.php" title="get optional caption for each image" tabindex="3">Next ❯</a></p></nav>'.
		'</footer>';
$scr = <<< SCR1
      <script>
window.onload = function() {
           ...
           $("input[type = 'submit']").click(function(){
               ...
               if (parseInt(fileUpload.get(0).files.length) > panelcount){
                  alert("You are only allowed to upload " + panelcount + " Comic image Alt text files!");
