...
&nbsp;<a href="mailto:itzbobwright@gmail.com">Bob Wright.</a>&nbsp;Last modified <!--#echo var="LAST_MODIFIED" --></footer></main>';
} else {
...
$offline = false;
	...
	</svg></span>&nbsp;<a href="mailto:itzbobwright@gmail.com">Bob Wright.</a>&nbsp;Last modified <!--#echo var="LAST_MODIFIED" --></footer></main>';
	} else {
...
if((file_exists("/home/bitnami/includes/panelcount.txt")) && (file_exists("/home/bitnami/includes/options.txt"))) {
$panelcount = (file_get_contents("/home/bitnami/includes/panelcount.txt"));
		$_SESSION['panelcount'] = $panelcount;
 }
...
if(getenv('QUERY_STRING') != '') {
	...
	// if both oauth_id and email_id are not set look for query
		if((array_key_exists('oauth', $pstrings)) && (file_exists("/home/bitnami/includes/options.txt"))) {
			$oauth_id = $pstrings['oauth'];
			$_SESSION['oauth_id'] = $oauth_id;
		} else {
			$_SESSION['oauth_id'] = '';
		}
		if((array_key_exists('email', $pstrings)) && (file_exists("/home/bitnami/includes/options.txt"))) {
			$email_id = $pstrings['email'];
	...
	if((array_key_exists('Comic', $pstrings)) && (file_exists("/home/bitnami/includes/options.txt"))) {
		...
		if(isset($_SESSION['oauth_id']) && ($_SESSION['oauth_id'] != '')) {
			$thisauthID = $_SESSION['oauth_id']; }
		if(isset($_SESSION['email_id']) && ($_SESSION['email_id'] != '')) {
			$thisauthID = $_SESSION['email_id']; }
				//echo '<h1>'.$thisauthID.'</h1><br>';
			$_SESSION['ComicFoldername'] = $ComicFoldername;
			$_SESSION['cardTitle'] = $ComicFoldername;
			if(file_exists('/home/bitnami/Comics/htdocs/'.$ComicFoldername.'/'.$ComicFoldername.'.php')) {
				...
				if( preg_match_all('/('.preg_quote($thisauthID,'/').')/i', $configFileContents, $matches)){
...
'Enter a Comic Title up to 64 characters in length. This title will appear below the card image on the gallery card. On the card, title text may wrap to multiple lines. The Title may contain alphabetic and numeric characters and spaces along with commas, underlines, and dashes. Space characters will be stripped from the Title to create a Comic name for the comic book page URL.<br>'.'<label>A Comic Title (required):<br><input type="text" name="cardTitle" size="64" value="'.$cardTitle.'" tabindex="1"></label>'.
'<br><input type="submit" value="Apply" tabindex="2">'.
'</form></section><br>';
...
'This URL will be the hosted comic base address. Its value is set by the application, it is an absolute URL that will become part of the page URL and will be used as the canonical URL in the HTML "head section". It is used along with the Comic name to create an OG URL for use when the comic is shared on Facebook. You may not change this value.<br>'.
'<label>The hosted comic base address (set by Storybook):<br><input type="text" name="siteurl" size="72" value="'.$siteurl.'" tabindex="-1"></label>'.
'</form></section><br>';
}
...
'<label>The hosted comic URL (set by Storybook):<br><input disabled type="text" name="siteurl" size="72" value="'.$siteurl.$Comicname.'.html" tabindex="-1"></label>'.
'</form></div></section><br>';
...
'<label>The Comic Subtitle (optional):<br><input type="text" name="cardSubtitle" size="64" value="'.$cardSubtitle.'" tabindex="3"></label>'.
'<br><input type="submit" value="Apply" tabindex="4">'.
'</form></section><br>';
...
'<label>The Gallery Card Text (required):<br><textarea name="cardText" rows="16" cols="64" tabindex="5">'.$cardText.'</textarea></label>'.
'<br><input type="submit" value="Apply" tabindex="6">'.
'</form></section><br>';
...
'<label>The comic Background image (optional):<br><input type="text" name="bkgndimage" size="64" value="'.$pageimage.'" hidden tabindex="-1"></label>'.
'<br><input type="submit" value="Select an Image" tabindex="7">'.
'</form></section><br>';
...
'<label>The Gallery card image (required):<br><input type="text" name="pageimage" size="64" value="'.$pageimage.'" hidden tabindex="-1"></label>'.
'<br><input type="submit" value="Select an Image" tabindex="8">'.
'</form></section><br>';
...
'<label>The Gallery Card Image Alt Text (required):<br><textarea name="cardAlt" rows="5" cols="64" tabindex="9">'.$cardAlt.'</textarea></label>'.
'<br><input type="submit" value="Apply" tabindex="10">'.
'</form></section><br>';
...
'<label>The Comic Category (required):<br><input type="text" name="category" size="32" value="'.$category.'" tabindex="11"></label>'.
'<br><input type="submit" value="Apply" tabindex="12">'.
'</form></section><br>';
...
'<label>The Comic Script Writer (optional):<br><input type="text" name="scriptname" size="32" value="'.$scriptname.'" tabindex="15"></label>'.
'<br><input type="submit" value="Apply" tabindex="16">'.
'</form></section>';
...
'<label>The Penciler (optional):<br><input type="text" name="pencilsname" size="32" value="'.$pencilsname.'" tabindex="17"></label>'.
'<br><input type="submit" value="Apply" tabindex="18">'.
'</form></section>';
...
'<label>The Inker (optional):<br><input type="text" name="inksname" size="32" value="'.$inksname.'" tabindex="19"></label>'.
'<br><input type="submit" value="Apply" tabindex="20">'.
'</form></section>';
...
'<label>The Colors Artist (optional):<br><input type="text" name="colorsname" size="32" value="'.$colorsname.'" tabindex="21"></label>'.
'<br><input type="submit" value="Apply" tabindex="22">'.
'</form></section>';
...
'<label>The Letterer (optional):<br><input type="text" name="lettersname" size="32" value="'.$lettersname.'" tabindex="23"></label>'.
'<br><input type="submit" value="Apply" tabindex="24">'.
'</form></section>';
...
'<label>The Comic Publisher (optional):<br><input type="text" name="publisher" size="32" value="'.$publisher.'" tabindex="25"></label>'.
'<br><input type="submit" value="Apply" tabindex="26">'.
'</form></section><br>';
...
'<label>The Comic intended Audience (required):<br><input type="text" name="audience" size="32" value="'.$audience.'" tabindex="27"></label>'.
'<br><input type="submit" value="Apply" tabindex="28">'.
'</form></section><br>';
...
'<label>The Comic Author for copyright (required):<br><input type="text" name="artistname" size="32" value="'.$artistname.'" tabindex="29"></label>'.
'<br><input type="submit" value="Apply" tabindex="30">'.
'</form></section><br>';
...
'<label>A contact email for the Comic (required):<br><input type="text" name="cardemail" size="64" value="'.$cardemail.'" tabindex="31"></label>'.
'<br><input type="submit" value="Apply" tabindex="32">'.
'</form></section><br>';
//'<hr class="new3">';
...
echo $cardcode;
			...
			echo
            '<img class="bd-placeholder-img card-img-top" alt="'.$cardAlt.'" width="100%" height="auto" src="../Comics/'.$Comicname.'/'.$cardImage.'">';
			}
			echo
			'<a class="mask rgba-white-light" title="Open the '.$cardTitle.' Comic" href="./index.php#anchor1" hreflang="en" tabindex="-1"></a>'.
			'</div>'.
			'<!--Card content-->'.
            '<div class="card-body">'.
			 '<!--Title-->'.
...
echo
'<section class="d-flex col-sm-11 flex-column align-items-center shadow-md #b0bec5 blue-grey lighten-5 px-sm-0 msgBox">'.
'<h3 id="CfgSave">Configuration and Gallery Card file Actions</h3>'.
...
if($_SESSION['usedSavedConfig'] == 1) { //because of 'Comic' query string, already checked ownership
	...
	'<label>Check to preserve any present content:&emsp;<input class="checkbox" type="checkbox" id="saveCheckbox" name="saveCheckbox" value="0" unchecked tabindex="33"></label><br>'.
	'<label>Save the Configuration and Card data:&emsp;<input type="submit" value="Apply" tabindex="34"></label></form><br>';
} else {
	if(!(isset($_SESSION['Comicname']))) {
		echo
		...
		if(!(file_exists('/home/bitnami/Comics/htdocs/'.$Comicname.'/'.$Comicname.'.php'))) {
			...
			'<label style="opacity: 0;">Check to preserve any present content:&emsp;<input class="checkbox" type="hidden" id="saveCheckbox" name="saveCheckbox" value="1" checked tabindex="33"></label><br>'.
			'<label>Save the Configuration and Card data:&emsp;<input type="submit" value="Apply" tabindex="34"></label></form><br>';
		} else {
			...
			if( preg_match_all('/('.preg_quote($thisauthID,'/').')/i', $configFileContents, $matches)){
				//print_r($matches);
			echo
			'<p>Configuration Data file exists with matching auth_id '.$thisauthID.'</p>'.
			'<form id="saveconfigDisplay" action="./saveConfig.php" method="post">'.
			...
			'<label>Save the Configuration and Card data:&emsp;<input type="submit" value="Apply" tabindex="34"></label></form><br>';
				...
				'<p>Configuration Data file exists but auth_id '.$thisauthID.' does not match. You may not modify this Comic.</p>';
			}
		}
	}
}
...
echo
	...
	'<nav><p><a id="prevpagebutton" href="./Portal.php" title="return to the logon portal" tabindex="36">❮ Previous</a>&emsp;&copy; 2020 by&nbsp;<span><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 40.000000 40.000000" preserveAspectRatio="xMidYMid meet">
	<g transform="translate(0.000000,40.000000) scale(0.100000,-0.100000)"
	fill="#000000" stroke="none">
	<path d="M97 323 l-97 -78 0 -122 0 -123 200 0 201 0 -3 127 -3 127 -93 73
	c-52 40 -97 73 -101 73 -4 0 -51 -35 -104 -77z m184 -9 c43 -30 79 -59 79 -63
	...
	-51z"/>
		...
		echo date ("F d Y H:i:s.", getlastmod()).'&emsp;<a id="nextpagebutton" href="./getComic.php" title="get the Comics images" tabindex="35">Next ❯</a></p></nav>'.
		'</footer>';
echo
'</main>';
}}
